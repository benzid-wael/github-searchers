...
describe('/api/search', () => {
    test('respond 200 to valid search request', async () => {
        ...
        // when / then
        client.post('/api/search').send(payload).expect(200);
    });
    ...
    test('respond 412 when search text is too short', async () => {
        ...
        // when / then
        await client.post('/api/search').send(payload).expect(412);
    });
    ...
    test('respond 405 when for blacklisted HTTP methods', async () => {
        ...
        const payload = { searchText: 'ts', searchType: 'repository' };
        // when / then
        await client.get('/api/search').expect(405);
    });
    ...
    test('respond 400 when for ivalid payload', async () => {
        ...
        const payload = {};
        // when / then
        await client.post('/api/search').send(payload).expect(400);
    });
});
